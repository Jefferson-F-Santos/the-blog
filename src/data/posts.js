import sidePost from '../assets/images/sidePost.png';
import post1 from '../assets/images/post1.png';
import post2 from '../assets/images/post2.png';
import post3 from '../assets/images/post3.png';

export const dataPosts = [
    {
        id: 1,
        data: 'Setembro 26, 2019',
        image: sidePost,
        title: 'Começo de um sonho',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nibh nibh eu in aliquet ut adipiscing neque. Sed volutpat aenean sit vitae, sed tristique nibh nibh eu in aliquet ut adipiscing neque. Sed volutpat aenean sit vitae, sed tristique. Sed volutpat aenean.',
    },
    {
        id: 2,
        data: 'Junho 24, 2003',
        image: post1,
        title: 'Conheça as principais técnicas para conseguir uma vaga internacional em programação',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nibh nibh eu in aliquet ut adipiscing neque. Sed volutpat aenean sit vitae, sed tristique.',
    },
    {
        id: 3,
        data: 'Janeiro 15, 2004',
        image: post2,
        title: 'Veja a evolução do Front-end na prática',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nibh nibh eu in aliquet ut adipiscing neque. ',
    },
    {
        id: 4,
        data: 'Abril 17, 2003',
        image: post1,
        title: '10 dicas para conseguir a vaga desejada em React JS',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nibh nibh eu in aliquet ut adipiscing neque. Sed volutpat aenean sit vitae, sed tristique.',
    },
    {
        id: 5,
        data: 'Agosto 05, 2000',
        image: post2,
        title: 'Deixe seu código mais semântico com essas dicas',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nibh nibh eu in aliquet ut adipiscing neque. Sed volutpat aenean sit vitae, sed tristique.',
    },
    {
        id: 6,
        data: 'Agosto 06, 2001',
        image: post3,
        title: 'Use essas dicas nas suas aplicações mobile e evolua no Front-end',
        description: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nibh nibh eu in aliquet ut adipiscing neque. Sed volutpat aenean sit vitae, sed tristique.',
    }
]